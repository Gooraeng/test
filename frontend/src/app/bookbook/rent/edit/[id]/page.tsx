...
export default function EditBookPage({ params }: EditBookPageProps): React.JSX.Element {
    ...
    useEffect(() => {
        const fetchBookDetail = async (): Promise<void> => {
            ...
            try {
                const response = await fetch(`/api/bookbook/rent/${id}`);
    ...
    useEffect(() => {
        if (bookImage) {
            ...
            if (bookDetail?.bookImage) {
                ...
                const fullImageUrl = bookDetail.bookImage.startsWith('http') ? bookDetail.bookImage : `https://bookbook.fly.dev${bookDetail.bookImage}`;
    ...
    const handleBookSearch = async (pageNumber: number): Promise<void> => {
        ...
        const backendSearchApiUrl = `/api/api/v1/bookbook/searchbook?query=${encodeURIComponent(searchQuery)}&start=${pageNumber}`;
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>): Promise<void> => {
        ...
        if (bookImage) {
            ...
            try {
                const imageUploadRes = await fetch("/api/api/v1/bookbook/upload-image", {
                    method: "POST",
                    body: imageFormData,
                });
        ...
        try {
            const res = await fetch(`/api/bookbook/rent/edit/${id}`, {
                method: "PUT",
                credentials: "include",
